...
const RegistroInstitucion = () => {
  ...
  const checkUserPermission = async (email: string) => {
    const response = await hasPermission(email, 'createOwn', 'organizationRequest');
  ...
  const onSubmit = async (values: Record<string, any>) => {
    ...
    console.log(descriptionsGenericas);
            ...
            >
              {({ setFieldValue, errors, touched }) => (
                <Form>
                  ...
                  {files.length > 0 &&
                    <GenericInputList elements={files} onDelete={handleDeleteFile} someDescriptions={descriptionsGenericas} handleTextChange={setDescriptionsGenericas} />
                  }
                  <br />
                  <br />
